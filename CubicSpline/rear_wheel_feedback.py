...
def closed_loop_prediction(cx, cy, cyaw, ck, speed_profile, goal,ax,ay):
...
def main():
    ...
    data = pd.read_csv('shortest_path_points.csv')
    data = data.loc[data.groupby(data.index//2).xr.idxmax(), :]
    ax = np.flip(np.array(data.xr))
    ay = np.flip(np.array(data.yr))
    goal = [ax[-1], ay[-1]]
